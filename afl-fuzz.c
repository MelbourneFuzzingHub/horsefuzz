...
// HorseFuzz
EXP_ST u32* funcids_bits;             /* SHM with function bitmap         */
static u32* fids;                     /* Functions ID                     */
static char** funcs;                  /* Functions name                   */
static int funcs_len = 0;
static u32* task;
static u32 task_len = 0;
static u8 para_mode;
EXP_ST u8 *task_file;

EXP_ST u8  virgin_bits[MAP_SIZE],     /* Regions yet untouched by fuzzing */
           covered_bits[MAP_SIZE],    /* Regions covered in task          */
...
struct queue_entry {
  ...
  u8  cal_failed,                     /* Calibration failed?              */
      ...
      no_instr;                       /* no instrumentation - horsefuzz   */
...
// HorseFuzz: for debugging
void DEBUG (char const *fmt, ...) {
  static FILE *f = NULL;
  if (f == NULL) {
    u8 *fn = alloc_printf("%s/horsefuzz.log", out_dir);
    f = fopen(fn, "w");
    ck_free(fn);
  }
  va_list ap;
...
// HorseFuzz: apply mask to trace_bits
void apply_mask() {
  for (int i = 0; i < MAP_SIZE; i++) {
    if (trace_bits[i]) {
      if (covered_bits[i] == 0) {
        int idx = found(task, funcids_bits[i], task_len);
        if (idx < 0)
          trace_bits[i] = 0;
        else
          covered_bits[i] = 1;
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  // HorseFuzz
  memset(funcids_bits, 0, MAP_SIZE * sizeof(u32));
  MEM_BARRIER();
  ...
  // HorseFuzz
  if (para_mode) apply_mask();

  tb4 = *(u32*)trace_bits;

  // HorseFuzz: disable this optimization because
  // it might change the mapping from the trace_bits to the funcids_bits
  // i.e., the bit indexes in trace_bits might change
  if (!para_mode) {
#ifdef WORD_SIZE_64
...
static u8 fuzz_one(char** argv) {
  ...
  //HorseFuzz
  if (queue_cur->no_instr) return 1;
...
static char** get_qemu_argv(u8* own_loc, char** argv, int argc) {
  ...
  SAYF("\n" cLRD "[-] " cRST
       ...
       "    already have the binary installed, you may need to specify HF_PATH in the\n"
       "    environment.\n\n"
...
int main(int argc, char** argv) {
  ...
  SAYF(cCYA "afl-fuzz HorseFuzz " cBRI VERSION cRST " by <lcamtuf@google.com>\n");
  ...
  // HorseFuzz
  memset(covered_bits, 0, MAP_SIZE);
  parse_functions();
